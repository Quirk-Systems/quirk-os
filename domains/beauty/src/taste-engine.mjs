...
export function derivePreferenceEvidence({ choice, options }) {
  ...
  for (const other of rejected) {
    ...
    for (const key of differingKeys) {
      comparisons.push({ other, key, denominator: differingKeys.length * rejected.length });
    }
  }
...
export function rankRecommendations({ actorId, purpose, candidates, evidence, generatedAt, ttlHours = 72 }) {
  const scopedEvidence = evidence.filter((item) => item.actorId === actorId && item.purpose === purpose);
  const expiresAt = new Date(new Date(generatedAt).getTime() + ttlHours * 60 * 60 * 1000).toISOString();
...
export function proposeGraphUpdate({ graph, recommendation, outcome, proposedAt, ttlHours = 24 }) {
  ...
  const multiplier = outcome.kind === "preferred" ? 0.15 : outcome.kind === "rejected" ? -0.15 : 0.05;
  ...
  const expiresAt = new Date(new Date(proposedAt).getTime() + ttlHours * 60 * 60 * 1000).toISOString();
  const seed = { actorId: graph.actorId, purpose: graph.purpose, recommendationId: recommendation.id, outcomeId: outcome.id, expectedGraphRevision: graph.revision, deltas };
...
export function decideGraphUpdate({ proposal, actorId, purpose, decision, humanConfirmed, reason, corrections = [], decidedAt, ttlHours = 2 }) {
  const result = {
    ...
    corrections,
    decidedAt,
    expiresAt: new Date(new Date(decidedAt).getTime() + ttlHours * 60 * 60 * 1000).toISOString(),
  };
  ...
  if (decision === "revise") {
    requireInvariant(corrections.length > 0, "gate.revision_without_correction", "A revised decision requires explicit corrected deltas.");
  }
  return result;
}

export function applyApprovedGraphUpdate({ graph, proposal, decision, appliedAt }) {
  ...
  for (const change of deltas) {
    const current = edgeMap.get(change.feature) ?? {
    ...
    const nextStrength = typeof change.setStrength === "number"
      ? change.setStrength
      : current.strength + change.delta;
  ...
  const nextGraph = {
    ...
    edges: [...edgeMap.values()].sort((left, right) => left.feature.localeCompare(right.feature)),
  };
  ...
  return {
    ...
    receipt: {
      ...
      receiptWriter: "candidate-local-adapter; replace with quirk.core.evidence",
    },
  };
}
